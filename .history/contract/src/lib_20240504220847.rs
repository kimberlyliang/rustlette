...
impl Roulette {
    ...
    pub fn end_game(&mut self) {
        ...
        assert!(env::block_timestamp() / 1_000_000_000 > self.end_time, "The game has not yet ended");

        // Generating a pseudo-random index based on block timestamp and hash
        let num_bidders = self.bidders.len() as usize;
        let hash = env::block_height(); // Using block index as part of randomness source
        let random_index = ((env::block_timestamp() + hash) as usize % num_bidders) as u64;
        
        let winner_id = self.bidders.get(random_index).unwrap();  // Safe unwrap because of previous checks
        ...
        env::log_str(format!("Winner: {}. Payout: {}.", winner_id, payout).as_bytes());
    }
